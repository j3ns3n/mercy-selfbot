...
module.exports = {
  ...
  execute: (bot, msg, args) => {
    if (args.length > 0) {
      const filtered = Object.keys(bot.commands).filter(
        c => args[0] === c || bot.commands[c].commands.indexOf(args[0]) > -1
      );
        ...
        try {
          ...
          msg.edit(
            "**Successfully reloaded command `" +
              bot.commands[filtered[0]].commands[0] +
              "`.**"
          );
        } catch (error) {
          msg.edit(
            "**An error occured while assigning command to self. Restoring in-memory command to before reload.**\n```js\n" +
              util.inspect(error) +
              "```"
          );
          delete bot.commands[filtered[0]];
