...
module.exports = {
  ...
  usage: "help [command]",
  description:
    "View a list of commands or get information about a specific command.",
  category: "Information",
  execute: (bot, msg, args) => {
    if (args.length > 0) {
      const command = Object.keys(bot.commands).filter(
        c => bot.commands[c].commands.indexOf(args[0]) > -1
      );
      if (command.length > 0) {
        const embed = new RichEmbed()
          ...
          .addField(
            "Aliases",
            bot.commands[command[0]].commands.length > 1
              ? bot.commands[command[0]].commands.slice(1).join(", ")
              : "No Aliases"
          )
      ...
      Object.keys(bot.commands).forEach(command => {
        if (bot.commands[command].category in helplist) {
          helplist[bot.commands[command].category] +=
            "\n" +
            bot.commands[command].commands[0] +
            " - " +
      ...
      const embed2 = new RichEmbed()
        ...
        .setColor(0x5b8dea)
        .setDescription(
          Object.keys(helplist)
            .map(command => "__" + command + "__\n" + helplist[command])
            .join("\n\n")
        )
        .setFooter(bot.config.strings.github)
        .setTimestamp();
