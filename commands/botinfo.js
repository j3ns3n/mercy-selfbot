    ...
    snekfetch
      ...
      .then(res => {
        const embed = new RichEmbed()
          ...
          .setThumbnail(
            `https://cdn.discordapp.com/avatars/${id}/a_${res.body.avatar}.webp`
          )
          .addField("ID", id)
          .addField(
            "Tag",
            `${res.body.username}#${res.body.discriminator}`,
            true
          )
          ...
          .addField(
            "Server Count",
            typeof res.body.server_count === "number"
              ? res.body.server_count
              : "This bot doesn't post server count!",
            true
          )
          .addField("Owner(s)", "<@" + res.body.owners.join(">\n<@") + ">")
          .addField(
            "Invite",
            "[Here](https://discordapp.com/oauth2/authorize?scope=bot&permissions=0&client_id=" +
              id +
              ")",
            true
          );
