...
module.exports = {
  ...
  execute: (bot, msg, args) => {
    if(args.length < 1) return msg.edit('**Please provide a user mention or ID**');
    if(!msg.guild) return msg.edit('**This command can only be used in a server**');
    let user = bot.users.get((msg.mentions.users.first() && msg.mentions.users.first().id) || args[0]);
    if(!user) return msg.edit('**A user with that ID was not found!**');
    let member = msg.guild.members.get(user.id);
    if(!member) return msg.edit('**A user with that ID was not found!**');
