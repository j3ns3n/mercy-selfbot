...
module.exports = (result, msg, args, bot) => {
  ...
  result = result
    .split(config.twitter.consumer_key)
    .join("-- TWITTER / CONSUMER KEY --");
  result = result
    .split(config.twitter.consumer_secret)
    .join("-- TWITTER / CONSUMER SECRET --");
  result = result
    .split(config.twitter.access_token)
    .join("-- TWITTER / ACCESS TOKEN --");
  result = result
    .split(config.twitter.access_token_secret)
    .join("-- TWITTER / ACCESS TOKEN SECRET --");

  if (result.length > 1900 - args.join(" ").length) {
    snekfetch
      .post("https://feed-the-wump.us/documents")
      .send(result)
      .then(body => {
        msg.edit(
          "**Input:**\n```js\n" +
            args.join(" ") +
            "```\n**Result was too long, generated hastebin link instead.\nhttps://feed-the-wump.us/" +
            body.body.key +
            ".json**"
        );
      })
